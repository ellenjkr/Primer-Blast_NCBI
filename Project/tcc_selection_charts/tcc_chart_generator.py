...
for item in os.listdir(os.getcwd()):
	if os.path.isdir(item):
		if item == 'tcc_selection':
			for file in os.listdir(item):
				if 'xlsx' in file:
					df = pd.read_excel(f'tcc_selection/{file}', sheet_name='Espécies')
			...
			# data.extend([pd.read_excel(f'tcc_selection/{file}', sheet_name='Espécies') for file in os.listdir(item) if 'xlsx' in file])
		else:
...
# pivot.sum().sort_values(ascending=False, inplace=False).to_csv('pivot.tsv', sep='\t')
